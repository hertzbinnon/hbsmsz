...
trap "STOP=1" SIGINT
# #####
gstd-client -p 5000 pipeline_create source0 rtmp2src location=rtmp://10.9.91.51/live/ch0 ! flvdemux name=demuxer demuxer. ! h264parse config-interval=-1 ! video/x-h264,stream-format=byte-stream,alignment=nal  ! queue  ! mpegtsmux name=muxer alignment=7 ! queue ! tcpserversink host=127.0.0.1 port=12341 demuxer. ! aacparse ! queue ! muxer.
gstd-client -p 5000 pipeline_create source1 rtmp2src location=rtmp://10.9.91.51/live/ch1 ! flvdemux name=demuxer demuxer. ! h264parse config-interval=-1 ! video/x-h264,stream-format=byte-stream,alignment=nal  ! queue  ! mpegtsmux name=muxer alignment=7 ! queue ! tcpserversink host=127.0.0.1 port=12342 demuxer. ! aacparse ! queue ! muxer.
gstd-client -p 5000 pipeline_create source2 rtmp2src location=rtmp://10.9.91.51/live/ch2 ! flvdemux name=demuxer demuxer. ! h264parse config-interval=-1 ! video/x-h264,stream-format=byte-stream,alignment=nal  ! queue  ! mpegtsmux name=muxer alignment=7 ! queue ! tcpserversink host=127.0.0.1 port=12343 demuxer. ! aacparse ! queue ! muxer.
gstd-client -p 5000 pipeline_create source3 rtmp2src location=rtmp://10.9.91.51/live/ch3 ! flvdemux name=demuxer demuxer. ! h264parse config-interval=-1 ! video/x-h264,stream-format=byte-stream,alignment=nal  ! queue  ! mpegtsmux name=muxer alignment=7 ! queue ! tcpserversink host=127.0.0.1 port=12344 demuxer. ! aacparse ! queue ! muxer.
gstd-client -p 5000 pipeline_create source4 rtmp2src location=rtmp://10.9.91.51/live/ch4 ! flvdemux name=demuxer demuxer. ! h264parse config-interval=-1 ! video/x-h264,stream-format=byte-stream,alignment=nal  ! queue  ! mpegtsmux name=muxer alignment=7 ! queue ! tcpserversink host=127.0.0.1 port=12345 demuxer. ! aacparse ! queue ! muxer.

gstd-client -p 5000 pipeline_create source5 rtmp2src location=rtmp://10.9.91.51/live/ch0 ! flvdemux name=demuxer demuxer. ! h264parse config-interval=-1 ! video/x-h264,stream-format=byte-stream,alignment=nal  ! queue  ! mpegtsmux name=muxer alignment=7 ! queue ! tcpserversink host=127.0.0.1 port=12346 demuxer. ! aacparse ! queue ! muxer.
gstd-client -p 5000 pipeline_create source6 rtmp2src location=rtmp://10.9.91.51/live/ch1 ! flvdemux name=demuxer demuxer. ! h264parse config-interval=-1 ! video/x-h264,stream-format=byte-stream,alignment=nal  ! queue  ! mpegtsmux name=muxer alignment=7 ! queue ! tcpserversink host=127.0.0.1 port=12347 demuxer. ! aacparse ! queue ! muxer.
gstd-client -p 5000 pipeline_create source7 rtmp2src location=rtmp://10.9.91.51/live/ch2 ! flvdemux name=demuxer demuxer. ! h264parse config-interval=-1 ! video/x-h264,stream-format=byte-stream,alignment=nal  ! queue  ! mpegtsmux name=muxer alignment=7 ! queue ! tcpserversink host=127.0.0.1 port=12348 demuxer. ! aacparse ! queue ! muxer.
gstd-client -p 5000 pipeline_create source8 rtmp2src location=rtmp://10.9.91.51/live/ch3 ! flvdemux name=demuxer demuxer. ! h264parse config-interval=-1 ! video/x-h264,stream-format=byte-stream,alignment=nal  ! queue  ! mpegtsmux name=muxer alignment=7 ! queue ! tcpserversink host=127.0.0.1 port=12349 demuxer. ! aacparse ! queue ! muxer.
gstd-client -p 5000 pipeline_create source9 rtmp2src location=rtmp://10.9.91.51/live/ch4 ! flvdemux name=demuxer demuxer. ! h264parse config-interval=-1 ! video/x-h264,stream-format=byte-stream,alignment=nal  ! queue  ! mpegtsmux name=muxer alignment=7 ! queue ! tcpserversink host=127.0.0.1 port=12340 demuxer. ! aacparse ! queue ! muxer.
