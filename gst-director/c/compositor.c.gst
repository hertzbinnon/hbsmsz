...
static void gst_debug_hertz();
void gst_debug_hertz(){
	g_print("what \n");
}
static void
    ...
    guint prop_id, const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    if (obj){
      ...
      if(!g_strcmp0("alpha",tokensvalue[0]))
      ...
      else if (!g_strcmp0("xpos",tokensvalue[0]))
        GST_COMPOSITOR_PAD(obj)->xpos = atoi(tokensvalue[1]);
      else if (!g_strcmp0("ypos",tokensvalue[0]))
        GST_COMPOSITOR_PAD(obj)->ypos = atoi(tokensvalue[1]);
      else if (!g_strcmp0("width",tokensvalue[0]))
        ...
        GST_WARNING_OBJECT (self, "not match ");
...
#if 1
  g_object_class_install_property (gobject_class, PROP_SINKPAD_PROPERTY,
      g_param_spec_string ("pad-set", "pad-set",
          "The property name of the pad to be set ",
          NULL, G_PARAM_READWRITE | GST_PARAM_CONTROLLABLE | G_PARAM_STATIC_STRINGS));
#endif
